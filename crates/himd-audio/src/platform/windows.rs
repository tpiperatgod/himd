//! Windows audio backend using cpal (WASAPI) + rodio.
//!
//! Provides native audio capture and playback on Windows without any
//! external shell commands or dependencies.
...
/// Probe the default input device on Windows using cpal/WASAPI.
pub fn probe_input_device() -> InputDeviceDiagnostics {
    ...
    match host.default_input_device() {
        Some(device) => {
            ...
            // Try building with i16 first (matching capture path)
            let init_ok = device
                .build_input_stream(
                    &config,
...
/// Probe the default output device on Windows using cpal/WASAPI.
...
/// Process a chunk of i16 samples: accumulate, run VAD.
fn process_capture_chunk(
    ...
    all.extend_from_slice(i16_samples);

    let mut pend = pending.lock().unwrap();
    pend.extend_from_slice(i16_samples);
...
/// Capture audio from the default microphone on Windows using cpal/WASAPI.
///
/// Handles Windows sample format conversion into mono i16. Reuses the
/// existing VAD and WAV writer. Blocking — call from `spawn_blocking`.
pub fn capture_once_blocking(max_duration_sec: Option<f64>) -> Result<CaptureResult, AudioError> {
    ...
    // Query the device's actual default config to determine sample format
...
/// Play a local audio file through the default output device on Windows using rodio.
///
/// Blocks until playback completes. Maps playback failures to actionable AudioError messages.
